...
class SimpleSpeedPlanner:
    ...
    def collision_points_and_path_callback(self, collision_points_msg, local_path_msg):
        try:
            ...
            # Braking safety distance taken from each collision point
            collision_point_braking_distances = collision_points["distance_to_stop"]
 
            # collision point speed projected onto the path heading
            ...
            # Target distances: raw distance minus car front and braking buffer,
            # minus reaction-time margin that grows with object speed
            target_distances = (collision_point_distances
                                - self.distance_to_car_front
                                - collision_point_braking_distances
                                - self.braking_reaction_time * np.abs(collision_point_speeds))
 
            # full target velocity formula accounting for collision point speed
            ...
            # Collision point causing the minimum target velocity
            ...
            # Metadata distances from the raw array
            target_object_distance = collision_point_distances[idx] - self.distance_to_car_front
            stopping_point_distance = collision_point_distances[idx] - collision_point_braking_distances[idx]
